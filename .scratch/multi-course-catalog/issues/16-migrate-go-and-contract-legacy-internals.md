...
**Blocker:** `learning-records/0005…0008 → docs/learning-records/0005…0008` 已在 `d303618` 以四项 R100 搬迁完成，内容与路径均已保全；它不再是本票阻塞项。其余三处用户数据仍受保护：`docs/go-learning/README.md` 的未提交评测说明、`exercise/day0/**` 的当前 Notes/Evaluation，以及 `roadmap/src/data/course.json` 的用户状态更新。Day 0 旧 Notes/Evaluation 与现有规范 Learning Record 不是同一字节内容；在未获明确迁移授权前，删除或覆盖任一侧都会造成数据丢失，且不得把旧 Roadmap JSON 的私有路径/用户修改带入公开投影。最初 applied journal 只绑定第一阶段 89 项 copy/rewrite/delete；仓库外 append-only `go-together-ticket16-finalization.json` 已补充绑定 guarded baseline/plan/journal、135 个最终树条目、37 Lesson/Notes/Evaluation/Snapshot 等价、零 symlink/旧消费者、5173 不变量及 142 项反向操作，fingerprint `d9b160bc7b8c95de55e802814312d626a663238a40211606ef1c510195dca6a9`。该补救提供可独立复核的最终/回滚证据，但不伪称后续文件是在原迁移锁内安装。因此第 16 票保留受权限影响的迁移锁完整切换、当前 Day 0 数据的显式迁入与旧源删除未完成，第 17–19 票不得越过。
